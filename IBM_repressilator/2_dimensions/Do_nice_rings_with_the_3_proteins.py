...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 23:08:45 2020

@author: pablo
"""

# This is basically a copy of the scripts "5_division_modes_test_shape_and_rings" and "Division_mode_E_layer_importance"
# The difference here is that we plot the three proteins to show that rings of protein expression appear with the 3.
...
time_steps=51; time_step=0#numer of rounds and the initial round for the while loop
...
#All the options we want to try (here only one, but as I reused I keep the loops for simplicity, they can be removed)
modes=[3.1001]
in_cells=1
move_=[0]
death_rates=[0]
        ...
        for death_rate in death_rates:
            for div_prob in div_probs:
                ...
                while time_step<time_steps: #the simulation will run until the number of rounds previously set
                    ...
                    if time_step==50: #I  print the matrix fro every 100, 200, 300....round, whenever the reminder equals 0!
                        plt.imshow(plotable_cI(grid), cmap="magma", vmax=30000)
                        plt.title("time: " + format(time_step))
                        plt.colorbar()
                        plt.savefig("201019/cI" + format(mode) + "_"+ format(div_prob) + "_" +format(death_rate)+"_" + format(in_cells)+ "_"+ format(time_step)+ "_"+format(move_prob)+".png", transparent=True)
                        plt.show()
                    
                        plt.imshow(plotable_tetR(grid), cmap="magma", vmax=30000)
                        plt.title("time: " + format(time_step))
                        plt.colorbar()
                        plt.savefig("201019/tetR" + format(mode) + "_"+ format(div_prob) + "_" +format(death_rate)+"_" + format(in_cells)+ "_"+ format(time_step)+ "_"+format(move_prob)+".png", transparent=True)
                        plt.show()
                    
                        plt.imshow(plotable_lacI(grid), cmap="magma", vmax=30000)
                        plt.title("time: " + format(time_step))
                        plt.colorbar()
                        plt.savefig("201019/lacI" + format(mode) + "_"+ format(div_prob) + "_" +format(death_rate)+"_" + format(in_cells)+ "_"+ format(time_step)+ "_"+format(move_prob)+".png", transparent=True)
                        plt.show()
                        
                        plt.imshow(plotable_div(grid), cmap="magma")
                        plt.title("time: " + format(time_step))
                        plt.colorbar()
                        plt.savefig("201019/div" + format(mode) + "_"+ format(div_prob) + "_" +format(death_rate)+"_" + format(in_cells)+ "_"+ format(time_step)+ "_"+format(move_prob)+".png", transparent=True)
                        plt.show()
